...
function makeGUI() {
  ...
  d3.select("label[for='percTrainData'] .value").text(e2p(state.percTrainData || 0));
...
function addPlusMinusControl(x: number, layerIdx: number) {
  ...
  div.append("span").text(
    e2p(state.networkShape[i]) + " نورون"
  );
}
...
function updateHoverCard(type: HoverType, nodeOrLink?: nn.Node | nn.Link,
  ...
  let name = (type === HoverType.WEIGHT) ? "وزن" : "بایاس";
  hovercard.style({
...
function updateUI(firstStep = false) {
  ...
  d3.select("#loss-train").text(e2p(humanReadable(lossTrain)));
  d3.select("#loss-test").text(e2p(humanReadable(lossTest)));
  d3.select("#iter-number").text(e2p(addCommas(zeroPad(iter))));
...
function reset(onStartup = false) {
  ...
  d3.select("#layers-label").text("لایه‌ی پنهان");
  d3.select("#num-layers").text(e2p(state.numHiddenLayers));
